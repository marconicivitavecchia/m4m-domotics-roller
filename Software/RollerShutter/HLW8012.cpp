...
unsigned int HLW8012::getActivePower() {
    ...
    _power = (_power_pulse_width > 0) ? _power_multiplier / _power_pulse_width / 2 : 0; // freq = 1/(durata di una semionda *2)
    return _power;
}
//funzione personalizzata per rilevazione efficace fine corsa tapparella
unsigned int HLW8012::getExtimActivePower() {//works only with interrupts!
    unsigned long _extim_power_pulse_width = micros() - _last_cf_interrupt;
	unsigned long _p_pulse_width;
	if(_extim_power_pulse_width > _power_pulse_width){
		_p_pulse_width = _extim_power_pulse_width;	//currently existimated pulse width
	}else{
		_p_pulse_width = _power_pulse_width;  //last measured pulse width
	}	 
    _checkCFSignal();
    _power = (_p_pulse_width > 0) ? _power_multiplier / _p_pulse_width / 2 : 0; // freq = 1/(durata di una semionda *2)
    return _power;
}
//funzione personalizzata per rilevazione efficace fine corsa tapparella con ottimizzazione per frequenze (potenze) elevate
unsigned int HLW8012::getAvgdExtimActivePower() {//works only with interrupts!
    unsigned long _extim_power_pulse_width = micros() - _last_cf_interrupt;
	...
	if(_extim_p_pulse_width > _p_pulse_width){
		_p_pulse_width = _extim_p_pulse_width;	//currently existimated pulse width
	}
...
void ICACHE_RAM_ATTR HLW8012::cf_interrupt() {
    ...
    _pulse_count++;
	//ottimizazzioni per misure di potenze di grande valore
	_summed_pulse_width += _power_pulse_width;
	_avg_pulse_count++;
}
...
void HLW8012::_calculateDefaultMultipliers() {
    ...
    _power_multiplier = ( 1000000.0 * 128 * V_REF * V_REF * _voltage_resistor / _current_resistor / 48.0 / F_OSC ); //*FREQ --> Pot
}
